...
function OrderHistory() {
  ...
  const orderbyCurrentUsers = orderHistory.filter(
    (obj) => obj.Orderby === userDetail?.uid
  );
                      ...
                      {order.orders.map((item, idx) => (
                        ...
                        >
                          <span className="flex gap-2 items-center">
                            <img src={item.thumbnail} className="w-12 rounded-md" alt="item image"  /> {item.name} ({item.quantity})
                          </span>
