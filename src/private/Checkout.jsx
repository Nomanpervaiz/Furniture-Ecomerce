...
function Checkout() {
  ...
  const { cartItem, updateCartAfterOrder } = useContext(ContextOfCart);
  ...
  const orderRef = userDetail?.uid
    ? doc(collection(db, "orders"), userDetail.uid)
    : null;

  async function syncOrderWithFirestore(newOrder) {
    if (orderRef) {
      try {
        const orderData = {
          order: newOrder,
    ...
    const fetchOrder = async () => {
      if (orderRef) {
        try {
          const orderSnapshot = await getDoc(orderRef);
          if (orderSnapshot.exists()) {
            setOrder(orderSnapshot.data().order || []);
          } else {
            setOrder([]);
          }
        } catch (error) {
          message.error("Failed to load order data from Firestore");
        }
      }
    };
    fetchOrder();
  ...
  const handlePlaceOrder = () => {
    const hasValidValue = objectValues.every((value) => value.trim() !== "");
    if (!hasValidValue) {
    ...
    syncOrderWithFirestore(updatedItems);
